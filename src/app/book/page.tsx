...
export default function BookPage() {
  ...
  return (
    ...
    <div className="flex" style={{ height: "calc(100vh - 64px)" }}>
      {/* Left panel */}
      <aside className="flex-shrink-0 w-full md:w-[400px] h-full overflow-y-auto bg-white border-r border-gray-100 shadow-lg z-10 flex flex-col">
        {/* Header */}
        <div className="px-5 pt-5 pb-4 border-b border-gray-100">
          <p className="text-xs font-semibold text-blue-600 uppercase tracking-widest mb-1">Hyderabad · DGCA Zones Active</p>
          <h1 className="text-xl font-bold">Book a Drone Pilot</h1>
          <p className="text-xs text-gray-500 mt-1">Click anywhere on the map to set your shoot location</p>
        </div>

        {/* Step indicator */}
        <div className="flex items-center gap-1.5 px-5 py-2.5 bg-gray-50 border-b border-gray-100">
          {([1, 2, 3] as Step[]).map((s) => (
            <div key={s} className="flex items-center gap-1">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${step === s ? "bg-black text-white" : step > s ? "bg-green-500 text-white" : "bg-gray-200 text-gray-500"}`}>
                {step > s ? "✓" : s}
              </div>
        ...
        <div className="flex-1 px-5 py-4 flex flex-col gap-4">
          ...
          {step === 1 && (
            <>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Shoot Location</p>
                <div className={`flex items-center gap-2.5 rounded-xl border-2 px-3 h-11 transition-colors ${coords ? "border-black bg-black/5" : "border-dashed border-gray-300 bg-gray-50"}`}>
                  <MapPin className={`w-4 h-4 flex-shrink-0 ${coords ? "text-black" : "text-gray-400"}`} />
                  <span className={`text-sm flex-1 truncate ${coords ? "font-medium text-gray-900" : "text-gray-400"}`}>
                    {location || "Click on the map to pin your location"}
                  </span>
                  {coords && <span className="text-xs text-gray-400">{coords.lat.toFixed(3)},{coords.lng.toFixed(3)}</span>}
                </div>
              </div>
              ...
              {/* Map legend */}
              {!coords && (
                <div className="flex gap-3 text-xs text-gray-500 bg-gray-50 rounded-xl p-3">
                  <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-red-400 inline-block" />Red = Prohibited</div>
                  <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-yellow-400 inline-block" />Yellow = Permission</div>
                  <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-green-400 inline-block" />Green = Free</div>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {SERVICES.map((svc) => (
                    <button key={svc.id} onClick={() => setSelectedService(svc)}
                      className={`p-3 rounded-xl border-2 text-left transition-all ${selectedService?.id === svc.id ? "border-black bg-black text-white" : "border-gray-200 hover:border-gray-300 bg-white"}`}>
                      <div className="text-xl mb-1">{svc.icon}</div>
                      <p className="font-semibold text-sm">{svc.label}</p>
                      <p className={`text-xs mt-0.5 ${selectedService?.id === svc.id ? "text-gray-300" : "text-gray-500"}`}>{svc.description}</p>
                      <p className={`text-sm font-bold mt-1.5 ${selectedService?.id === svc.id ? "text-blue-300" : "text-blue-600"}`}>{inr(svc.price)}+</p>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {/* Date */}
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Shoot Date</p>
                <div className="relative">
                  <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                  <input type="date" min={today} value={date} onChange={(e) => setDate(e.target.value)}
                    className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-black transition-colors" />
                </div>
              </div>

              <button onClick={handleFindPilots} disabled={!selectedService || !coords}
                className="w-full py-3 bg-black text-white rounded-xl font-semibold text-sm flex items-center justify-center gap-2 hover:bg-gray-900 disabled:opacity-40 disabled:cursor-not-allowed transition-all mt-auto">
                Find Pilots Near Me <ChevronRight className="w-4 h-4" />
              </button>
              {!coords && <p className="text-xs text-center text-gray-400">Pin your location on the map first</p>}
            </>
          )}
          ...
          {step === 2 && (
            <div className="flex flex-col gap-3">
              <div className="flex flex-col items-center py-5 gap-3">
                ...
                <p className="text-sm font-medium text-gray-700">Searching pilots near {location}…</p>
                <p className="text-xs text-gray-400">{selectedService?.label} · {inr(selectedService?.price || 0)}</p>
              </div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Available Pilots</p>
              {PILOTS.map((p) => (
                <div key={p.id} className={`flex items-center gap-3 bg-gray-50 rounded-xl p-3 transition-all duration-500 ${appearedPilots.has(p.id) ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}>
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ background: p.color }}>{p.initials}</div>
                  <div className="flex-1">
                    <p className="font-semibold text-sm">{p.name}</p>
                    <p className="text-xs text-gray-500">⭐ {p.rating} · {p.distance}</p>
                  </div>
                  <span className="text-xs text-blue-600 font-semibold">Checking…</span>
                </div>
              ))}
            </div>
          )}
          ...
          {step === 3 && (
            <div className="flex flex-col gap-4">
              <div>
                <p className="text-xs font-semibold text-green-600 uppercase tracking-wide mb-0.5">✓ Pilot Matched</p>
                <h2 className="text-lg font-bold">Your Pilot is Ready</h2>
              </div>
              ...
              <div className="border-2 border-black rounded-2xl p-4">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-white font-bold text-xl flex-shrink-0" style={{ background: matchedPilot.color }}>{matchedPilot.initials}</div>
                  <div>
                    <p className="font-bold text-base">{matchedPilot.name}</p>
                    <p className="text-sm text-gray-500 flex items-center gap-1"><Star className="w-3.5 h-3.5 text-yellow-500 fill-yellow-500" /> {matchedPilot.rating} · {matchedPilot.distance}</p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {matchedPilot.certs.map((c) => (
                    <span key={c} className="flex items-center gap-1 text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded-full font-medium"><Shield className="w-3 h-3" />{c}</span>
                  ))}
                  {(zone?.zoneType === "red" || zone?.zoneType?.includes("yellow")) && (
                    <span className="flex items-center gap-1 text-xs bg-orange-50 text-orange-700 px-2 py-1 rounded-full font-medium">
                ...
                <div className="bg-gray-50 rounded-xl px-4 py-3 flex justify-between items-center">
                  <div>
                    <p className="text-xs text-gray-500">{selectedService?.icon} {selectedService?.label}</p>
                    <p className="text-xs text-gray-400">{date || "Date TBD"} · {location}</p>
                  </div>
                  <span className="text-lg font-bold">{inr(selectedService?.price || 0)}</span>
                </div>
              </div>

              {/* Other pilots */}
              <div>
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Switch Pilot</p>
                {PILOTS.map((p) => (
                  <div key={p.id} onClick={() => setMatchedPilot(p)}
                    className={`flex items-center gap-3 rounded-xl p-2.5 mb-1.5 cursor-pointer border-2 transition-all ${matchedPilot.id === p.id ? "border-black bg-black/5" : "border-transparent hover:bg-gray-50"}`}>
                    <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ background: p.color }}>{p.initials}</div>
                    <div className="flex-1">
                      <p className="font-semibold text-sm">{p.name}</p>
                      <p className="text-xs text-gray-500">⭐ {p.rating} · {p.distance}</p>
                    </div>
                    {matchedPilot.id === p.id && <CheckCircle className="w-4 h-4 text-black" />}
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-2 mt-auto">
                {!booked ? (
                  <>
                    <button onClick={() => setBooked(true)} className="w-full py-3 bg-black text-white rounded-xl font-semibold text-sm flex items-center justify-center gap-2 hover:bg-gray-900 transition-colors">
                      Accept & Book <ChevronRight className="w-4 h-4" />
                    </button>
                    <button onClick={() => { setAppearedPilots(new Set()); setStep(2); }} className="w-full py-2.5 border border-gray-300 rounded-xl text-sm font-medium text-gray-600 flex items-center justify-center gap-2 hover:border-gray-500 transition-colors">
